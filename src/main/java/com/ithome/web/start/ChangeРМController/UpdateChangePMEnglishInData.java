...
@WebServlet("/UpdateChangePMEnglishInData")
public class UpdateChangePMEnglishInData extends HttpServlet {
    ...
    private String changePMRussian=null;
    ...
    private void gotoNextPage(HttpServletRequest request, HttpServletResponse response, String message) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("/WEB-INF/ChangePMEnglish.jsp").forward(request, response);
    }

    private void getEnglishChangePM() {
        changeРМList = changeРМDao.getChangeРМInEnglish();
    }

    private int CreateNewTextInData(int id) {
        return changeРМDao.UpdateChangeРМEng(CreateObjectOfText(),id);
    }

    private ChangeРМ CreateObjectOfText() {
        return new ChangeРМ(fullText,changePMRussian);
    }
    ...
    private void getRussianText(int id) {
        ...
        for (int i = 0; i <changeРМList.size() ; i++) {
            changePMRussian  = changeРМList.get(i).getChangeРМRus();
        }
