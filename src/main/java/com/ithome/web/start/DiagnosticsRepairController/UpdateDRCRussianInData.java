package com.ithome.web.start.DiagnosticsRepairController;
...
@WebServlet("/UpdateDRCRussianInData")
public class UpdateDRCRussianInData extends HttpServlet {
    private SessionChecker checker = new SessionChecker();
    private String username = null;
    private AdminChecker adminChecker = new AdminChecker();
    private int adminId = 0;
    private List<Admin> adminList = new ArrayList<>();
    private List<DiagnosticsRepair> diagnosticsRepairList = new ArrayList<>();
    private DiagnosticsRepairDao diagnosticsRepairDao = new DiagnosticsRepairDao();
    private int id =0;
    ...
    private void getEnglishText(int id) {
        diagnosticsRepairList = diagnosticsRepairDao.getDRCInEnglishById(id);
        for (int i = 0; i <diagnosticsRepairList.size() ; i++) {
            CEnglish  = diagnosticsRepairList.get(i).getDiagnosticsRepairEng();
        }
    ...
    private void gotoNextPage(HttpServletRequest request, HttpServletResponse response, String message) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("/WEB-INF/DiagnosticsRepair/DRRussian.jsp").forward(request, response);
    }
    ...
    private void setRequestToCUpdatePage(HttpServletRequest request) {
        ...
        request.setAttribute("DiagnosticsRepairList", diagnosticsRepairList);
    }

    private void getEnglishC() {
        diagnosticsRepairList = diagnosticsRepairDao.getDiagnosticsRepairInRussian();
    }


    private int CreateNewTextInData(int id) {
        return diagnosticsRepairDao.UpdateDiagnosticsRepairRus(CreateObjectOfText(),id);
    }

    private DiagnosticsRepair CreateObjectOfText() {
        return new DiagnosticsRepair(CEnglish,fullText);
    }
