...
@WebServlet("/UpdateRPBDiscsRussianInData")
public class UpdateRPBDiscsRussianInData extends HttpServlet {
    ...
    private void getEnglishText(int id) {
        replacementPadsBrakeDiscsList = replacementPadsBrakeDiscsDao.getRPBDiscsInEnglishById(id);
        for (int i = 0; i <replacementPadsBrakeDiscsList.size() ; i++) {
            CEnglish  = replacementPadsBrakeDiscsList.get(i).getReplacementPadsBrakeDiscsEng();
        }
    ...
    private void gotoNextPage(HttpServletRequest request, HttpServletResponse response, String message) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("/WEB-INF/RPBDiscs/RPBDiscsRussian.jsp").forward(request, response);
    }
    ...
    private void getRussian() {
        replacementPadsBrakeDiscsList = replacementPadsBrakeDiscsDao.getReplacementPadsBrakeDiscsInRussian();
    }


    private int CreateNewTextInData(int id) {
        return replacementPadsBrakeDiscsDao.UpdateReplacementPadsBrakeDiscsRus(CreateObjectOfText(),id);
    }

    private ReplacementPadsBrakeDiscs CreateObjectOfText() {
        return new ReplacementPadsBrakeDiscs(CEnglish,fullText);
    }
