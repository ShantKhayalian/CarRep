...
@WebServlet("/UpdateDRCEnglishInData")
public class UpdateDRCEnglishInData extends HttpServlet {
    ...
    private void getRussianText(int id) {
        diagnosticsRepairList = diagnosticsRepairDao.getDRCInRussianById(id);
        for (int i = 0; i <diagnosticsRepairList.size() ; i++) {
            CRussian  = diagnosticsRepairList.get(i).getDiagnosticsRepairRus();
        }
    ...
    private void gotoNextPage(HttpServletRequest request, HttpServletResponse response, String message) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("/WEB-INF/DiagnosticsRepair/DREnglish.jsp").forward(request, response);
    }
    ...
    private void getEnglishC() {
        diagnosticsRepairList = diagnosticsRepairDao.getDiagnosticsRepairInEnglish();
    }


    private int CreateNewTextInData(int id) {
        return diagnosticsRepairDao.UpdateDiagnosticsRepairEng(CreateObjectOfText(),id);
    }

    private DiagnosticsRepair CreateObjectOfText() {
        return new DiagnosticsRepair(fullText,CRussian);
    }
